...
#     s_array = cuda.shared.array(shape=(scan_array_size), dtype=np.int32) # create and copy data to block-sum-array
...
def scan_array(arr):
    ...
    stream = cuda.stream()
    arr_dump = arr.copy_to_host(stream=stream)
    stream.synchronize()
    # try:
    ...
    #     np.savetxt("output/arr_dump.txt", arr_dump, delimiter =", ")
    #     print('Error with scan_preScan!')
    #     print(type(inst))
    #     return
    ...
    if prescan_blockspergrid == 1:
        ...
        d_block_sum = cuda.device_array(prescan_blockspergrid + prescan_blockspergrid%2, np.int32) # needs to be even
        ...
        if (sum_prescan_blockspergrid == 2): #cant be one. TODO Merge with  elif (prescan_blockspergrid <= scan_array_size):
            ...
            #block block sum
            threadsperblock = scan_max_threads
            d_block_block_sum = cuda.device_array(sum_prescan_blockspergrid + sum_prescan_blockspergrid%2, np.int32) # needs to be even
